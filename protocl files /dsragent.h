...
class CDStimers : public TimerHandler {
public:
  CDStimers(DSRAgent *a) : TimerHandler() { a_ = a;}
  void expire(Event *e);
...
//Beaconing Timers class 
//Beacontimers

class Beacontimers : public TimerHandler {
public:
  Beacontimers(DSRAgent *a) : TimerHandler() { a_ = a;}
  void expire(Event *e);
friend class DSRAgent;
protected:
  DSRAgent *a_;
...
// Delay timer Class 
class Delay : public TimerHandler {
public:
  Delay(DSRAgent *a) : TimerHandler() { a_ = a;}
  void expire(Event *e);
friend class DSRAgent;
protected:
  DSRAgent *a_;
...
// Child_node_reply Child_node_reply_timeout ;   Child_reply_waiting_time_out
class Child_node_reply : public TimerHandler {
public:
  Child_node_reply(DSRAgent *a) : TimerHandler() { a_ = a;}
  void expire(Event *e);
friend class DSRAgent;
protected:
  DSRAgent *a_;
...
//.............................................mycode
LIST_HEAD(my_ncache, struct My_Neighbor);
class Diff_Test {
        friend class DSRAgent;
        //friend class aodv_rt_entry;
 public:
        Diff_Test(nsaddr_t a,nsaddr_t cw,nsaddr_t an,nsaddr_t f,int lv) { s_id=a;cwrkr = cw,atnode=an, wfid=f;wnl=lv;}

 protected:
        LIST_ENTRY(Diff_Test) nb_link;
        nsaddr_t s_id;
        nsaddr_t cwrkr;
        nsaddr_t atnode;
        nsaddr_t wfid;
...
//Arun ==================== 201111
  ...
  void sendHello();
 struct  My_Neighbor*       nb_lookup(nsaddr_t id);
   void            nb_insert(nsaddr_t id,int);
   my_ncache         nbhead;
  struct   My_Neighbor*       nb_print();

	///arun  //my Code !!!!
	void send_B1_pkt(nsaddr_t starter_node );
	void send_B2_pkt(nsaddr_t receiver_node );
	void send_B3_pkt(nsaddr_t receiver_node );
	void send_Pick_up_pkt(nsaddr_t node_id);
	void send_me(nsaddr_t starter_node );
	void send_node_selection_pkt(nsaddr_t node_id);
	void send_Beacon_pkt();
	void send_offer_pkt(int offered_node_id);
	void send_Black_pkt();
	void send_reset_packet();
	void send_route_find_pkt(int responsible_node_for_fixing ,int problem_node );
	void send_route_rep_pkt(int responsible_node_for_fixing ,int problem_node  );
	void send_child_discover_pkt(int node_id);
	void send_NAC(int rcv_n);
	void neighbors(int i) ;
	void Change_Status(nsaddr_t node_id , int status_code);
	int get_node_status(int rcv_n);
	void Reduce_Nodes(int snd_node_id , int rcv_node_id );
	void print_the_whole_node_status();
	void start_CDS_timer(int node_id);
	int get_net_id();
	void Pick_up_a_Node();
	void print_whole_end_status();
	int get_total_nodes();
	void beaconing();
	void start_beacon_timer(int node_id);
	void start_delay_timer();
	void fixing(int responsible_node_for_fixing , int problem_node);
	int parent_node(int node_id);
	void connect(int responsible_node_for_fixing ,int problem_node );
	void neighbor_black_node_prepare();
	void start_child_reply_timer();
	void check_leaf_node();
	void send_live_pkt(int rcv_n);
...
#define TAP_BITMASK	(TAP_CACHE_SIZE - 1)
  ...
  //arun
  CDStimers timeout; // CDS grey node time outs 
  Beacontimers beacontimeout ; // time out for beaconing 
  Delay delaytimeout;  //delay for timer 
  Child_node_reply Child_node_reply_timeout ;
  ...
  friend class CDStimers;
  friend class Beacontimers;
  friend class Delay;
  friend class Child_node_reply;
